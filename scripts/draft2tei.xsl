...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:for-each select="descendant::div[@type eq 'song']">
            <xsl:variable name="nr"><xsl:number></xsl:number></xsl:variable>
            <xsl:variable name="author" select="descendant::author => replace('\s+', '')"/>
            <xsl:variable name="performer" select="person/persName => replace('\s+', '') => encode-for-uri()"/>
            <xsl:variable name="title" select="title => replace('\s+', '') => encode-for-uri()"/>
            <xsl:result-document href="{$author}/{$title}_{$performer}_{$nr}.xml">
                <TEI>
                    <teiHeader>
                        <fileDesc>
                            <titleStmt>
                                <title><xsl:value-of select="title"/></title>
                                <author><xsl:value-of select="person/persName"/></author>
                            </titleStmt>
                            <publicationStmt>
                                <authority>UNED</authority>
                            </publicationStmt>
                            <sourceDesc>
                                <bibl type="album" subtype="{replace(descendant::bibl/@subtype, '\s+', '')}">
                                    <title><xsl:value-of select="descendant::bibl/title"/></title>
                                    <publisher><xsl:value-of select="descendant::bibl/editor"/></publisher>
                                    <date>
